...
export default function MoreCards() {
  ...
  return (
    ...
    <Image src={trust} alt="" className="md:w-[40%] lg:w-[30%]" />
    </div>
  ...
  <div
    ...
    className={`${index + 1 + Math.floor((currentPage - 1) / 3) * 3 <= totalPages ? '' : 'hidden'} cursor-pointer border border-[#b8b6e7] rounded-full p-2 px-4 ${
      index + 1 + Math.floor((currentPage - 1) / 3) * 3 === currentPage ? 'bg-[#51b37e] text-white' : ''
    }`}
  ...
  >
    ...
    {/* Add the last button */}
    <div
      className={`cursor-pointer border border-[#b8b6e7] rounded-full p-2 px-4 ${
        totalPages === currentPage ? 'bg-[#51b37e] text-white' : ''
      }`}
