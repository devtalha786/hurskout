...
export default function FAQ({rounded}) {
  ...
  return (
    <div className={`${rounded ? 'rounded-2xl relative bottom-24 md:static' : ''} w-full  flex flex-col gap-y-10 justify-center items-center py-8  md:pb-16 bg-[#b8b6e7]`}>
      <h5 className="text-[35px] sm:text-[40px] md:text-[45px] lg:text-[60px] font-[900]  sm:w-[80%] md:w-[40%]  text-center  uppercase">Veel gestelde vragen</h5>
    <div className="flex flex-col justify-center items-center">
      ...
      {faq.map((contain) => (
          ...
          className={`${
              contain.isOpen ? 'h-auto ' : 'h-[75px] sm:h-[66px] md:h-[77px]'
          } ${contain.id > 0 ? 'border-t' : ''} transition-all px-10 duration-200 cursor-pointer flex flex-col  overflow-hidden w-[98%]  sm:w-[91%] md:w-[70%] lg:w-[57%] xl:w-[53%] md:py-4 pb-5 text-[17px] md:text-xl font-semibold bg-[#fbf4ea] rounded-2xl mb-4`}
          >
          <h1 className={`${contain.id === 0 ? ' mt-3.5 md:mt-0.5' : 'mt-6   md:mt-1.5'} flex items-center justify-between mb-3 `}>
               <span className="max-w-[90%]">{contain.question}</span>

              <IoIosArrowDown size={20} className={`${contain.isOpen ? '-rotate-180' : ''}`} />
          </h1>
